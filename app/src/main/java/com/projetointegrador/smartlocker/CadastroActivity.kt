...
class CadastroActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnCriarConta.setOnClickListener {
            ...
            if (!isOnline(this)) {
                val snackbar = Snackbar.make(it, "Conecte-se a internet", Snackbar.LENGTH_SHORT)
                snackbar.show()
            } else {
                ...
                if (nome.isNotEmpty() && cpf.isNotEmpty() && email.isNotEmpty()
                    ...
                    if (senha == confSenha) {
                        criarConta(nome, cpf, email, celular, dataNasc, senha, confSenha, it)
                    } else {
                    ...
                    if (nome.isEmpty()) {
                        binding.cadastroEditTextNome.setError("Coloque seu nome")
                    }
                    if (cpf.isEmpty()) {
                        binding.cadastroEditTextCPF.setError("Coloque seu cpf")
                    }
                    if (email.isEmpty()) {
                        binding.cadastroEditTextEmail.setError("Coloque seu email")
                    }
                    if (celular.isEmpty()) {
                        binding.cadastroEditTextCelular.setError("Coloque seu celular")
                    }
                    if (dataNasc.isEmpty()) {
                        binding.cadastroEditTextDataNasc.setError("Coloque sua data de nascimento")
                    }
                    if (senha.isEmpty()) {
                        binding.cadastroEditTextCriarSenha.setError("Coloque sua senha")
                    }
                    if (confSenha.isEmpty()) {
                        binding.cadastroEditTextConfirmeSenha.setError("Coloque sua senha")
                    }
                }
            }
            ...
            .addOnCompleteListener { cadastro ->
                if (cadastro.isSuccessful) {
                    ...
                    db.collection("usuarios").document(userId)
                        ...
                        }.addOnFailureListener { e -> // 'e' is the exception
                            val snackbar = Snackbar.make(it, "Um erro incomum ocorreu!", Snackbar.LENGTH_SHORT)
                            snackbar.show()
                        }
            }
        }
